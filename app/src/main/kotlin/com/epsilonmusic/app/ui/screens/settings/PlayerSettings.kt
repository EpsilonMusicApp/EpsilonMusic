...
import com.epsilonmusic.app.constants.EnableGoogleCastKey
...
import com.epsilonmusic.app.constants.SeekExtraSeconds
...
import com.epsilonmusic.app.ui.component.EnumDialog
...
highlightKey: String? = null) {
    ...
    val (dataSaverEnabled, onDataSaverEnabledChange) = rememberPreference(
        key = com.epsilonmusic.app.constants.DataSaverEnabledKey,
        defaultValue = false
    )
    ...
    val (playbackEngine, onPlaybackEngineChange) = rememberEnumPreference(
        com.epsilonmusic.app.constants.PlaybackEngineKey,
        defaultValue = com.epsilonmusic.app.constants.PlaybackEngine.AUTO
    )
    ...
    if (showDownloadQualityDialog) {
        EnumDialog(
            ...
            values = listOf(com.epsilonmusic.app.constants.DownloadQuality.YOUTUBE),
            valueText = {
                when (it) {
                    com.epsilonmusic.app.constants.DownloadQuality.YOUTUBE -> "YouTube Music (AAC/Default)"
                    else -> ""
                }
            }
        )
    ...
    if (showPlaybackEngineDialog) {
        EnumDialog(
            ...
            onSelect = {
                ...
                com.epsilonmusic.app.utils.YTPlayerUtils.playbackEngine = it
                showPlaybackEngineDialog = false
            },
            title = "Playback Engine",
            current = playbackEngine,
            values = listOf(
                com.epsilonmusic.app.constants.PlaybackEngine.POTOKEN,
                com.epsilonmusic.app.constants.PlaybackEngine.BRAVEPIPE,
                com.epsilonmusic.app.constants.PlaybackEngine.AUTO,
            ),
            valueText = {
                when (it) {
                    com.epsilonmusic.app.constants.PlaybackEngine.POTOKEN -> "PoToken (Recommended)"
                    com.epsilonmusic.app.constants.PlaybackEngine.BRAVEPIPE -> "BravePipe (NewPipe)"
                    com.epsilonmusic.app.constants.PlaybackEngine.AUTO -> "Auto (Try Both)"
                }
            },
            valueDescription = {
                when (it) {
                    com.epsilonmusic.app.constants.PlaybackEngine.POTOKEN -> "Uses WebView PoToken + CipherDeobfuscator. Most reliable and future-proof."
                    com.epsilonmusic.app.constants.PlaybackEngine.BRAVEPIPE -> "Uses NewPipe extractor for stream resolution. Lightweight but may break with YouTube updates."
                    com.epsilonmusic.app.constants.PlaybackEngine.AUTO -> "Tries PoToken first, falls back to BravePipe if it fails."
                }
            }
        )
    }
    ...
    isHighlighted = (highlightKey == stringResource(R.string.echo_equalizer)),
                    icon = painterResource(R.drawable.epsilon_equalizer),
