...
package com.epsilonmusic.app.epsilonmusic.updater
...
import com.epsilonmusic.app.epsilonmusic.updater.downloadmanager.UpdateDownloadWorker
import com.epsilonmusic.app.epsilonmusic.updater.downloadmanager.DownloadNotificationManager
...
import com.epsilonmusic.app.ui.component.ChangelogItem
import com.epsilonmusic.app.ui.component.leadingItemShape
import com.epsilonmusic.app.ui.component.middleItemShape
import com.epsilonmusic.app.ui.component.endItemShape
import com.epsilonmusic.app.ui.component.detachedItemShape
import com.epsilonmusic.app.ui.component.parseMarkdown
import com.epsilonmusic.app.ui.component.endItemShape
import com.epsilonmusic.app.ui.component.detachedItemShape
import com.epsilonmusic.app.ui.component.AnimatedActionButton
import com.epsilonmusic.app.ui.component.ExpressiveIconButton
import com.epsilonmusic.app.ui.component.ErrorSnackbar
...
sealed class EpsilonUpdateStatus {
    object Idle : EpsilonUpdateStatus()
    object Checking : EpsilonUpdateStatus()
    ...
    data class NoUpdate(val version: String) : EpsilonUpdateStatus()
    data class Error(val message: String) : EpsilonUpdateStatus()
}
...
fun UpdateScreen(navController: NavHostController) {
    ...
    var status by remember { mutableStateOf<EpsilonUpdateStatus>(EpsilonUpdateStatus.NoUpdate(BuildConfig.VERSION_NAME)) }
    ...
    fun triggerUpdateCheck() {
        status = EpsilonUpdateStatus.Checking
        scope.launch {
            ...
            checkForUpdate(
                ...
                onSuccess = { tag, isAvailable, changelog, size, date, description, imageUrl, apkUrl ->
                    ...
                    status = if (isAvailable) {
                        ...
                        EpsilonUpdateStatus.NoUpdate(tag)
                    }
                },
                onError = {
                    status = EpsilonUpdateStatus.Error(context.getString(R.string.cant_check_updates))
                }
            )
        }
    }
    ...
    Scaffold(
        topBar = {
            LargeTopAppBar(
                title = {
                    val titleText = if (status is EpsilonUpdateStatus.Available) {
                        buildAnnotatedString {
                            ...
                            withStyle(
                                ...
                                append((status as EpsilonUpdateStatus.Available).version)
                            }
                        }
                    } else {
            ...
            Surface(
                ...
                Column(
                    ...
                    when (val currentStatus = status) {
                        is EpsilonUpdateStatus.Idle, is EpsilonUpdateStatus.Checking, is EpsilonUpdateStatus.NoUpdate, is EpsilonUpdateStatus.Error -> {
                            AnimatedActionButton(
                                text = stringResource(R.string.check_for_update),
                                onClick = { triggerUpdateCheck() },
                                enabled = currentStatus !is EpsilonUpdateStatus.Checking && !isDownloading,
                                modifier = Modifier.fillMaxWidth()
                            )
                        }

                        is EpsilonUpdateStatus.Available -> {
                            Row(
                                modifier = Modifier.fillMaxWidth(),
                                ...
                                AnimatedActionButton(
                                    ...
                                    onClick = {
                                        if (isDownloadComplete) {
                                            ...
                                            val urlToDownload = currentStatus.apkUrl ?: "https://github.com/EpsilonMusicApp/EpsilonMusic/releases/download/${currentStatus.version}/epsilonmusic.apk"
        ...
        Box(
            ...
            LazyColumn(
                ...
                item {
                    val contentModifier = if (status is EpsilonUpdateStatus.Available) {
                    ...
                    Box(
                        ...
                        when (val currentStatus = status) {
                            is EpsilonUpdateStatus.Checking -> {
                                androidx.compose.material3.ContainedLoadingIndicator(
                                    modifier = Modifier.size(64.dp)
                                )
                            }

                            is EpsilonUpdateStatus.NoUpdate -> {
                                Column(
                                    horizontalAlignment = Alignment.CenterHorizontally
                                ) {
                                    Icon(
...
suspend fun checkForUpdate(
    ...
    withContext(Dispatchers.IO) {
        try {
            val url = URL("https://api.github.com/repos/EpsilonMusicApp/EpsilonMusic/releases/latest")
            ...
            if (shouldShow) {
                ...
                try {
                    val changelogUrl =
                        URL("https://github.com/EpsilonMusicApp/EpsilonMusic/releases/download/$tagWithPrefix/changelog.json")
